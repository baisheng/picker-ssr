...
class APlayer {
  ...
  setMusic (index) {
    ...
    if (this.isMobile && this.audio) {
      ...
      this.audio.addEventListener('play', () => {
        if (this.button.classList.contains('aplayer-play')) {
          ...
          setTimeout(() => {
            this.button.innerHTML = `<button type="button" class="aplayer-icon aplayer-icon-pause">` +
              this.getSVG('pause') +
              `     </button>`
          }, 100)
    ...
    // autoplay
    // console.log(this.option.autoplay)
    if (this.option.autoplay && !this.isMobile) {
      this.play()
    }
    // TODO: @basil 20170905 会自动播放，不知道为什么是创建两次也许是因为服务端渲染的问题
  ...
  play (time) {
    ...
    if (this.audio.paused) {
      // @basil 20170905 https://stackoverflow.com/questions/37136077/how-do-i-prevent-the-error-uncaught-in-promise-domexception-the-play-reque
      const playPromise = this.audio.play()
      if (playPromise !== null) {
        playPromise.catch(() => {
          console.log('audio puse exception')
          this.audio.play()
        })
      }
    }
